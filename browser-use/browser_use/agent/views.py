...
class AgentHistoryList(BaseModel):
	...
	def is_successful(self) -> bool:
		"""Check if the agent completed successfully based on the final action's success status."""
		if not self.history or not self.history[-1].result:
			return False
			
		# If the last action has success=True, the task is successful
		last_result = self.history[-1].result[-1]
		return last_result.success == True
